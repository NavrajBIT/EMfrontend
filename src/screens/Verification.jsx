...
function Verification({ navigation, route }) {
  ...
  const handleVerifyOtp = async () => {
    ...
    if (response && response.data?.token) {
      ...
      await AsyncStorage.setItem(
        'is_new',
        JSON.stringify(response?.data?.is_new),
      );
      await AsyncStorage.setItem(
        'user_type',
  ...
  const handleSubmit = async () => {
    ...
    const response = await fetch(
      `${env.url}/auth/resend_otp?phone_number=${route.params.number}`,
      {
        method: 'GET',
        headers: {
  ...
  return (
    <ScrollView style={styles.container}>
      ...
      <Text style={[styles.heading, { marginTop: 50 }]}>
      ...
      <Text
        style={[
          styles.heading,
          { color: PRIMARY_COLOR },
        ]}>{`${route?.params?.number.substring(
          0,
          2,
        )}******${route?.params?.number.substring(8, 10)}`}</Text>
      ...
      <View
        ...
        <OTPTextInput
          ...
          textInputStyle={[
            ...
            { borderWidth: 1, borderBottomWidth: 1, margin: 4 },
          ]}
          tintColor="#000000"
          offTintColor="#000000"
      ...
      <CustomButton
        title={'Verify'}
        customStyle={otp.length === 6 ? styles.button : styles.disabledButton}
      ...
      />
      {
        <TouchableOpacity
          ...
          <Text
            ...
            {count !== 0 ? 'Resend verification code' : 'Resend Code'}
          </Text>
          {count !== 0 && (
            <Text
              style={{
